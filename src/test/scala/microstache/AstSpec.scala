...
class AstSpec extends ScalaCheckSuite {
    ...
    def nonEmptyStr(gen: Gen[Char]): Gen[String] = Gen.posNum[Int].flatMap(i => Gen.stringOfN(i, gen))

    val genIdentifier: Gen[Ast.Identifier] = Gen.nonEmptyListOf(nonEmptyStr(Gen.alphaChar)).map(l => Ast.Identifier(NonEmptyList.fromListUnsafe(l)))
    ...
    val genTemplate: Gen[Template] = Gen.listOf(genTerm).map { lst =>
